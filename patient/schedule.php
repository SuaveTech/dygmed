...
<body>
    ...
    $absolute_path = $userfetch['profile_picture'] ?? null;
    ...
    // $sqlmain= "select * from schedule where hospital_id='$hospital_id' inner join doctor on schedule.docid=doctor.docid where schedule.scheduledate>='$today'  order by schedule.scheduledate asc";

    $stmt = $database->prepare("SELECT * 
                         FROM schedule 
                         INNER JOIN doctor ON schedule.docid = doctor.docid 
                         WHERE schedule.hospital_id = ? 
                           AND schedule.scheduledate >= ? 
                         ORDER BY schedule.scheduledate ASC");
    $stmt->bind_param("ss", $hospital_id, $today);
    $stmt->execute();
            ...
            <tr>
                ...
                <td>
                    <form action="" method="post" class="header-search">
                        ...
                        $list12 = $database->query("select DISTINCT * from  schedule where hospital_id='$hospital_id' GROUP BY title;");
